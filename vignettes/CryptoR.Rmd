...
```{r }
customDF <- cg_coins_df(per_page = 50, pages = 5)
```
...
analysis. We can create a linear model for each symbol's price and then extract 
the slope to give us a rough idea of the 7-day trend.  

We can use `apply` on each row of the tibble and send data to the 
...
Again, we'll use the coins' or symbols' slope measurement but weigh it by
market capitalization. We can then get a sense of the market cap weighted 
...
Lastly, let's look at the 10 cryptocurrencies which 
**by weighted market cap slopes** are affecting the market the greatest. This 
could be **upwards or downwards** pressure! Here we'll just look at the 
absolute values of weighted slopes.
...
dplyr::top_n(coinsWeighting[-2:-5], 10, absVal)
```
